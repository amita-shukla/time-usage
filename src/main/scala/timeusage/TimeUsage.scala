...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val workingActivities: List[Column] = columnNames.filter(column => column.startsWith("t05") || column.startsWith("t1805"))
      .map(elem => new Column(elem))
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(row =>
      TimeUsageRow(row.getAs("working"), row.getAs("sex"), row.getAs("age"), row.getAs("primaryNeeds"), row.getAs("work"), row.getAs("other")))
